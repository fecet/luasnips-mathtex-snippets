...
M.setup = function(opts)
	...
	local markdown_normal = {}

	for _, snip in ipairs(require("luasnip-latex-snippets/normal_wA_markdown")) do
		snip.condition = pipe({ not_math })
		table.insert(markdown_normal, snip)
	end

	ls.add_snippets("tex", snippets, { default_priority = 0 })

	ls.add_snippets("tex", autosnippets, {
		type = "autosnippets",
		default_priority = 0,
	})
	ls.add_snippets("tex", tex_normal, {
		type = "autosnippets",
		default_priority = 0,
	})
	...
	ls.add_snippets("markdown", markdown_normal, {
		type = "autosnippets",
		default_priority = 0,
	})
end
